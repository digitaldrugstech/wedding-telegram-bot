...
async def job_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as db:
        ...
        flavor_texts = {
            ...
            "banker": [
                f"Обслужил {random.randint(15, 30)} клиентов",
                f"Провёл {random.randint(10, 25)} транзакций",
                f"Одобрил {random.randint(5, 15)} кредитов",
            ],
            "infrastructure": [
                f"Собрал {random.randint(20, 40)} ресурсов",
                f"Построил {random.randint(5, 15)} объектов",
                f"Отремонтировал {random.randint(3, 10)} зданий",
            ],
            "court": [
                f"Рассмотрел {random.randint(3, 8)} дел",
                f"Вынес {random.randint(2, 6)} приговоров",
                f"Провёл {random.randint(1, 4)} слушания",
            ],
            "culture": [
                f"Провёл {random.randint(2, 5)} ивентов",
                f"Организовал {random.randint(1, 3)} мероприятия",
        ...
        if scammed:
            response = (
                ...
                f"🎰 ВАС НАЕБАЛИ ДРУЗЬЯ НА КАЗИНО !\n\n"
